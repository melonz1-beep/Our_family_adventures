...
// Build 0008 — working preview interactions
const chatForm = document.getElementById('chatForm');
const chatInput = document.getElementById('chatInput');
const chatThread = document.getElementById('chatThread');
function formatMentions(text){
  return escapeHtml(text).replace(/(^|\s)@([A-Za-z][\w-]*)/g, '$1<b>@$2</b>');
}
chatForm?.addEventListener('submit', (e)=>{
  ...
  msg.innerHTML = `<span class="mini-avatar">ML</span><p>${formatMentions(text)}</p>`;
...
document.addEventListener('click', (e)=>{
  const removeTag = e.target.closest('[data-tag] button');
  if(removeTag){ removeTag.closest('[data-tag]').remove(); }
  if(e.target.closest('#addTagBtn')){
    alert('Tag People preview: select a name from People and place it on the photo. Tags can now be removed with ×.');
  }
});

document.getElementById('photoUpload')?.addEventListener('change', e=>{
  if(e.target.files.length) alert(`${e.target.files.length} photo(s) selected. Photo upload storage will connect in the Firebase build.`);
});
document.getElementById('videoUpload')?.addEventListener('change', e=>{
  if(e.target.files.length) alert(`${e.target.files.length} video(s) selected. Video upload storage will connect in the Firebase build.`);
});
