...
const SECURITY_RULES_VERSION='9.4.3-fcm-sender';
...
async function queuePushNotification(targetUids,text,kind='trips',options={}){
  try{
    ...
    await firebase.database().ref(`${basePath()}/notificationQueue/${queueId}`).set(payload);
    return true;
  }catch(error){console.error('Push queue failed',error);messagingError='Sender queue: '+String(error?.message||error);localStorage.setItem('ofa-fcm-error',messagingError);return false}
}
...
function testNotification(){if(Notification.permission!=='granted'){toast('Enable browser notifications first');return}notify('Real push test from Our Family Adventures',true,uid);toast('Real FCM push test queued. Put the app in the background for the phone banner.')}
