...
(function(){
  ...
  function drawCover(ctx,img,x,y,w,h){const r=Math.max(w/img.width,h/img.height);const nw=img.width*r,nh=img.height*r;ctx.save();roundRect(ctx,x,y,w,h,12);ctx.clip();ctx.drawImage(img,x+(w-nw)/2,y+(h-nh)/2,nw,nh);ctx.restore();}
  ...
  window.exportScrapbookPage=async function(id){
    ...
    try{
      ...
      for(const e of [...p.elements].sort((a,b)=>(a.z||0)-(b.z||0))){
        const x=e.x/100*PAGE_W,y=e.y/100*PAGE_H,w=e.w/100*PAGE_W,h=e.h/100*PAGE_H;
        if(e.type==='photo'){ ctx.fillStyle='white'; roundRect(ctx,x,y,w,h,18); ctx.fill(); const src=srcForPhoto(e); if(src){ const img=await loadImage(src); const r=Math.max((w-20)/img.width,(h-20)/img.height); const nw=img.width*r,nh=img.height*r; ctx.save(); roundRect(ctx,x+10,y+10,w-20,h-20,12); ctx.clip(); ctx.drawImage(img,x+10+(w-20-nw)/2,y+10+(h-20-nh)/2,nw,nh); ctx.restore(); }}
...
/* 6.0 requested feature lock: scrapbook workflow, dashboard tools, live weather, analytics */
(function(){
  const OFA_VERSION_60='6.0';
  data.settings=data.settings||{};
  data.settings.adminPin='1218';
  if(!data.settings.notifications)data.settings.notifications={};
  ['chat','trips','birthdays','meals','weather','scrapbook','journal','votes','budget','reservations'].forEach(k=>{if(typeof data.settings.notifications[k]!=='boolean')data.settings.notifications[k]=false;});

  // Cache/version indicator for troubleshooting.
  window.OFA_VERSION='6.0';

  function ensureTripArrays60(){
    data.trips.forEach(t=>{
  ...
  // Safer family name helper: only asks when someone votes/checks an item, not just when the dashboard renders.
  window.familyNameForAction=function(){
    if(window.firebaseUser&&firebaseUser.email)return firebaseUser.email;
    let name=localStorage.getItem('ofa_family_user')||localStorage.getItem('ofa_voter_name')||'';
    if(!name){name=prompt('Your name');if(name){localStorage.setItem('ofa_family_user',name.trim());localStorage.setItem('ofa_voter_name',name.trim());}}
    return (name||'Family Member').trim();
  };
  window.currentVoter=function(){return window.familyNameForAction();};

  // Weather code labels for the live weather line.
  function weatherLabel(code){
    const map={0:'Clear',1:'Mostly clear',2:'Partly cloudy',3:'Cloudy',45:'Fog',48:'Fog',51:'Light drizzle',53:'Drizzle',55:'Heavy drizzle',61:'Light rain',63:'Rain',65:'Heavy rain',71:'Light snow',73:'Snow',75:'Heavy snow',80:'Rain showers',81:'Rain showers',82:'Heavy showers',95:'Thunderstorm'};
    return map[code]||'Forecast';
  }
  async function geocode60(q){
  ...
  window.getWeatherFor=async function(place){
    const g=await geocode60(place);
    if(!g)throw new Error('Location not found');
    const url=`https://api.open-meteo.com/v1/forecast?latitude=${g.latitude}&longitude=${g.longitude}&current=temperature_2m,precipitation,weather_code,wind_speed_10m&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=auto`;
    const r=await fetch(url); const j=await r.json(); const c=j.current||{}, d=j.daily||{};
    return `${weatherLabel(c.weather_code)} • ${Math.round(c.temperature_2m)}°F now • high ${Math.round(d.temperature_2m_max?.[0]||0)}° / low ${Math.round(d.temperature_2m_min?.[0]||0)}° • rain chance ${d.precipitation_probability_max?.[0]??0}% • wind ${Math.round(c.wind_speed_10m||0)} mph`;
  };
  window.refreshTripWeather=async function(id){
    const t=data.trips.find(x=>String(x.id)===String(id)); if(!t)return;
    const place=t.address||t.destination; if(!place)return toast('Add a trip address or destination first.');
    try{toast('Checking live weather...'); t.weather=await window.getWeatherFor(place); t.weatherUpdatedAt=new Date().toISOString(); save(); render(); notify(`Weather updated for ${t.name}: ${t.weather}`,'weather');}
    catch(e){console.error(e); toast('Live weather could not load. Check the address or destination.');}
  };
  const refreshBtn=document.getElementById('refreshWeatherBtn');
  if(refreshBtn)refreshBtn.onclick=()=>{const t=byNextTrip()||data.trips[0]; if(t)window.refreshTripWeather(t.id); else toast('Add a trip first.');};

  // Delete helpers for planning tools.
  window.removeBudgetItem=(tripId,itemId)=>{const t=data.trips.find(x=>String(x.id)===String(tripId)); if(!t)return; t.budgetItems=(t.budgetItems||[]).filter(x=>String(x.id)!==String(itemId)); save(); render();};
  window.removeReservation=(tripId,itemId)=>{const t=data.trips.find(x=>String(x.id)===String(tripId)); if(!t)return; t.reservations=(t.reservations||[]).filter(x=>String(x.id)!==String(itemId)); save(); render();};
  window.removeItinerary=(tripId,itemId)=>{const t=data.trips.find(x=>String(x.id)===String(tripId)); if(!t)return; t.itinerary=(t.itinerary||[]).filter(x=>String(x.id)!==String(itemId)); save(); render();};
  window.removeSharedNote=(tripId,itemId)=>{const t=data.trips.find(x=>String(x.id)===String(tripId)); if(!t)return; t.sharedNotes=(t.sharedNotes||[]).filter(x=>String(x.id)!==String(itemId)); save(); render();};

  // Force the requested wording/buttons for scrapbook workflow.
  function labelScrapbookButtons60(){
    const saveBtn=document.getElementById('savePage'); if(saveBtn)saveBtn.textContent='Create Draft Page';
    const bookBtn=document.getElementById('printBook'); if(bookBtn)bookBtn.textContent='PDF / Print Full Book';
    const exportBtn=document.getElementById('exportBook'); if(exportBtn)exportBtn.textContent='Export Last Page JPEG';
  }

  const previousRender60=render;
  render=function(){ensureTripArrays60(); previousRender60(); labelScrapbookButtons60(); if(typeof renderAnalytics==='function')renderAnalytics();};
  ensureTripArrays60(); labelScrapbookButtons60(); render();
})();

/* 6.0.2 fixes: scrapbook photo saving, local storage compression, and layout safety */
(function(){
  window.OFA_VERSION='6.0.2';
  const MAX_LOCAL_IMG=620;
  const LOCAL_QUALITY=.48;

  function dataUrlBytes(s){return typeof s==='string'&&s.startsWith('data:image/')?Math.ceil(s.length*0.75):0;}
  function shrinkDataUrl(src,max=MAX_LOCAL_IMG,quality=LOCAL_QUALITY){
    return new Promise(resolve=>{
      if(!src||!String(src).startsWith('data:image/')) return resolve(src);
      const img=new Image();
      img.onload=()=>{
        try{
  ...
  function fileToSmallImage(file,max=MAX_LOCAL_IMG,quality=LOCAL_QUALITY){
    return new Promise(resolve=>{
      const reader=new FileReader();
      reader.onload=()=>shrinkDataUrl(reader.result,max,quality).then(resolve);
      reader.onerror=()=>resolve('');
      reader.readAsDataURL(file);
    });
  }

  const oldSaveMediaFile = typeof saveMediaFile==='function' ? saveMediaFile : null;
  window.saveMediaFile = saveMediaFile = async function(file,path,max=MAX_LOCAL_IMG,quality=LOCAL_QUALITY){
    if(firebaseReady&&firebaseUser&&oldSaveMediaFile){
      return oldSaveMediaFile(file,path,max,quality);
    }
    if(file.type&&file.type.startsWith('video')){toast('Video selected. Sign in to Firebase to save/play videos in the app.');return '';}
    return await fileToSmallImage(file,MAX_LOCAL_IMG,LOCAL_QUALITY);
  };

  async function compactExistingPhotos(){
    let changed=false;
    for(const m of (data.memories||[])){
      for(const media of (m.media||[])){
        if(media.type==='image'&&dataUrlBytes(media.src)>420000){
          media.src=await shrinkDataUrl(media.src,MAX_LOCAL_IMG,LOCAL_QUALITY);
          changed=true;
        }
      }
    }
    for(const p of (data.pages||[])){
      if(Array.isArray(p.photos)){
        for(let i=0;i<p.photos.length;i++){
          if(dataUrlBytes(p.photos[i])>420000){p.photos[i]=await shrinkDataUrl(p.photos[i],MAX_LOCAL_IMG,LOCAL_QUALITY);changed=true;}
        }
      }
      if(Array.isArray(p.elements)){
        for(const e of p.elements){
          if(e.type==='photo'&&dataUrlBytes(e.src)>420000){e.src=await shrinkDataUrl(e.src,MAX_LOCAL_IMG,LOCAL_QUALITY);changed=true;}
        }
      }
    }
    if(changed){try{localStorage.setItem(KEY,JSON.stringify(data));toast('Photos compressed so scrapbook can save on this device.');}catch(e){toast('Device storage is still full. Use Settings → Download Backup, then clear old browser site data or sign in to Firebase.');}}
  }
  ...
  save = function(){
    try{localStorage.setItem(KEY,JSON.stringify(data));}
      ...
      console.warn(e);
      compactExistingPhotos().then(()=>{try{localStorage.setItem(KEY,JSON.stringify(data));}catch(err){console.warn(err);}});
      toast('This device storage is full. I compressed photos; try saving again, or use Firebase for large photo/video saving.');
    }
    try{queueFirebaseSave();}catch(e){}
  };

  function allPhotos(){return (data.memories||[]).flatMap(m=>(m.media||[]).filter(x=>x.type==='image'&&x.src).map(x=>({...x,memoryTitle:m.title||'Memory',trip:m.trip||''})));}
  function selectedPhotos(){const ids=new Set(selectedScrapPhotos||[]);return allPhotos().filter(x=>ids.has(x.id));}
  function boxes(layout,count){
    if(layout==='feature')return [{x:6,y:19,w:58,h:52},{x:68,y:19,w:25,h:16},{x:68,y:38,w:25,h:16},{x:68,y:57,w:25,h:16},{x:6,y:74,w:27,h:14},{x:36,y:74,w:27,h:14},{x:66,y:74,w:27,h:14}];
    if(layout==='collage')return [{x:6,y:19,w:42,h:31},{x:51,y:19,w:42,h:20},{x:51,y:42,w:20,h:23},{x:73,y:42,w:20,h:23},{x:6,y:53,w:42,h:22},{x:51,y:68,w:42,h:20},{x:6,y:78,w:26,h:12},{x:35,y:78,w:26,h:12}];
    if(layout==='freeform')return Array.from({length:Math.max(count,1)},(_,i)=>({x:8+(i%3)*28,y:20+Math.floor(i/3)*22,w:26,h:18}));
  ...
  function themeBg(theme){return theme==='Sunset'?'sunset':theme==='Rose Gold'?'roseGold':theme==='Lighthouse'?'lighthouseClean':theme==='Beach Day'?'sand':'seaGlass';}

  window.createDraftScrapbookPage=function(){
    const photos=selectedPhotos();
    if(!photos.length&&!$('pageTitle')?.value.trim()&&!$('pageNote')?.value.trim())return toast('Select photos or add a title/note first.');
    const layout=$('pageLayout')?.value||'grid';
    const frame=$('pageFrame')?.value||'soft';
    const b=boxes(layout,photos.length);
    window.draftPage={
      id:'draft',title:$('pageTitle')?.value.trim()||'Scrapbook Page',note:$('pageNote')?.value.trim()||'',
      theme:$('pageTheme')?.value||'Beach Day',layout,frame,bg:$('pageBg')?.value||themeBg($('pageTheme')?.value),createdAt:new Date().toISOString(),
      elements:photos.map((ph,i)=>({id:uid(),type:'photo',src:ph.src,mediaId:ph.id,x:b[i]?.x??8,y:b[i]?.y??20,w:b[i]?.w??40,h:b[i]?.h??22,frame,z:10+i}))
    };
    const sticker=($('pageSticker')?.value||'🐚').split(' ')[0];
    const text=($('pageTextBox')?.value||'').trim();
    if(sticker)draftPage.elements.push({id:uid(),type:'sticker',text:sticker,x:7,y:10,w:9,h:7,z:80});
    if(text)draftPage.elements.push({id:uid(),type:'text',text,x:11,y:83,w:38,h:7,z:90});
    if(typeof renderDraft==='function')renderDraft(); else render();
    toast('Draft created. Rearrange, then create scrapbook page.');
  };

  window.saveDraftScrapbookPage=function(){
    if(!window.draftPage)return toast('Create a draft page first.');
    const page=JSON.parse(JSON.stringify(window.draftPage));
    page.id=uid(); page.createdAt=new Date().toISOString();
    page.photos=(page.elements||[]).filter(e=>e.type==='photo').map(e=>e.src).filter(Boolean);
    data.pages.push(page);
    window.draftPage=null; selectedScrapPhotos=[];
    ['pageTitle','pageNote','pageTextBox'].forEach(i=>{if($(i))$(i).value=''});
    logActivity(`Scrapbook page saved: ${page.title}`,'scrapbook');
    save(); render(); show('scrapbook'); toast('Scrapbook page saved with photos');
  };

  const oldRenderChoices = window.renderScrapbookChoices;
  window.renderScrapbookChoices=function(){
    const box=$('scrapbookMemoryChoices'); if(!box)return;
    const imgs=allPhotos();
    const count=`<p class="selectedCount">${selectedScrapPhotos.length} photo${selectedScrapPhotos.length===1?'':'s'} selected.</p>`;
    box.innerHTML=imgs.length?`${count}<div class="memorySelect">${imgs.map(x=>`<button type="button" class="memoryChoice ${selectedScrapPhotos.includes(x.id)?'selected':''}" onclick="toggleScrapPhoto('${x.id}')"><img src="${x.src}" alt="${esc(x.name||'photo')}"><small>${esc(x.memoryTitle)}</small></button>`).join('')}</div><div class="buttonRow"><button id="createDraftPageBtn" class="primary" type="button">Create Draft Page</button><button id="clearSelectedPhotosBtn" class="secondary" type="button">Clear Selected</button></div>`:'<p>No memory photos yet. Save photos in Memories first, then come back here.</p>';
    const c=$('createDraftPageBtn'); if(c)c.onclick=()=>window.createDraftScrapbookPage();
    const clr=$('clearSelectedPhotosBtn'); if(clr)clr.onclick=()=>{selectedScrapPhotos=[];window.draftPage=null;renderScrapbookChoices();try{renderDraft();}catch(e){}};
  };

  // Run after load to repair old oversized local data.
  setTimeout(()=>compactExistingPhotos().then(()=>{try{render();}catch(e){}}),900);
})();


/* 6.0.3 final fixes: safe toast, real scrapbook draft, Canva-style controls, clearer Firebase setup */
(function(){
  window.OFA_VERSION='6.0.3';
  const PAGE_W=850, PAGE_H=1100;

  function setSplashClass(){document.body.classList.toggle('splashShowing', !!document.querySelector('#splash.active'));}
  const oldShow603 = typeof show==='function' ? show : null;
  if(oldShow603){ window.show = show = function(id){ oldShow603(id); setSplashClass(); }; }
  setSplashClass();
  const begin=document.getElementById('beginBtn');
  if(begin){ const old=begin.onclick; begin.onclick=function(e){ if(typeof old==='function')old.call(this,e); setTimeout(setSplashClass,30); }; }

  const oldToast603 = typeof toast==='function' ? toast : null;
  window.toast = toast = function(msg){
    const t=document.getElementById('toast');
    if(!t){ if(oldToast603)oldToast603(msg); return; }
    const text=String(msg||'');
    t.textContent=text.includes('This device storage is full')
      ? 'Device storage is full. Use Settings → Download Backup, clear old site data, or connect Firebase for large photos/videos.'
      : text;
    t.classList.remove('hidden');
    clearTimeout(t._timer);
    t._timer=setTimeout(()=>t.classList.add('hidden'), text.length>80?5200:3200);
    setSplashClass();
  };

  function hasRealFirebaseConfig(){
    const c=window.firebaseConfig||{};
    return !!(window.OFA_FIREBASE_ENABLED && c.apiKey && !String(c.apiKey).includes('PASTE_') && c.appId && !String(c.appId).includes('PASTE_') && c.projectId && !String(c.projectId).includes('PASTE_'));
  }
  const oldLogin603 = typeof firebaseLoginFlow==='function' ? firebaseLoginFlow : null;
  window.firebaseLoginFlow = firebaseLoginFlow = async function(){
    if(!hasRealFirebaseConfig()){
      toast('Firebase is not connected yet. Open firebase-config.js, paste your Web App config, then re-upload.');
      try{show('settings');}catch(e){}
      return;
    }
    if(oldLogin603) return oldLogin603();
  };
  const loginBtn=document.getElementById('loginBtn'); if(loginBtn)loginBtn.onclick=firebaseLoginFlow;

  function allMemoryPhotos(){
    return (data.memories||[]).flatMap(m=>(m.media||[]).filter(x=>x.type==='image'&&x.src).map(x=>({
      id:x.id, src:x.src, name:x.name||'photo', memoryTitle:m.title||'Memory', trip:m.trip||''
    })));
  }
  function selectedPhotoObjects603(){
    const ids=new Set(selectedScrapPhotos||[]);
    return allMemoryPhotos().filter(x=>ids.has(x.id));
  }
  function pct(n){return (Math.round(Number(n||0)*10)/10)+'%';}
  function frameValue(){return document.getElementById('pageFrame')?.value||'soft';}
  function bgValue(){
    const bg=document.getElementById('pageBg')?.value;
    if(bg)return bg;
    const theme=document.getElementById('pageTheme')?.value||'';
    return theme==='Sunset'?'sunset':theme==='Rose Gold'?'roseGold':theme==='Lighthouse'?'lighthouseClean':theme==='Beach Day'?'sand':'seaGlass';
  }
  function layoutBoxes603(layout,count){
    if(layout==='feature')return [{x:6,y:18,w:58,h:50},{x:68,y:18,w:25,h:16},{x:68,y:37,w:25,h:16},{x:68,y:56,w:25,h:16},{x:6,y:72,w:27,h:14},{x:36,y:72,w:27,h:14},{x:66,y:72,w:27,h:14}];
    if(layout==='collage')return [{x:6,y:18,w:42,h:31},{x:51,y:18,w:42,h:20},{x:51,y:41,w:20,h:23},{x:73,y:41,w:20,h:23},{x:6,y:52,w:42,h:22},{x:51,y:67,w:42,h:20},{x:6,y:77,w:26,h:12},{x:35,y:77,w:26,h:12}];
    if(layout==='freeform')return Array.from({length:Math.max(count,1)},(_,i)=>({x:7+(i%3)*29,y:19+Math.floor(i/3)*22,w:27,h:18}));
    return Array.from({length:Math.max(count,1)},(_,i)=>({x:6+(i%2)*46,y:18+Math.floor(i/2)*23,w:42,h:20}));
  }
  function makeDraft603(){
    const photos=selectedPhotoObjects603();
    if(!photos.length&&!document.getElementById('pageTitle')?.value.trim()&&!document.getElementById('pageNote')?.value.trim()){
      toast('Select photos or add a title/note first.'); return null;
    }
    const layout=document.getElementById('pageLayout')?.value||'grid';
    const frame=frameValue();
    const boxes=layoutBoxes603(layout,photos.length);
    const elements=photos.map((ph,i)=>({id:uid(),type:'photo',src:ph.src,mediaId:ph.id,x:boxes[i]?.x??8,y:boxes[i]?.y??20,w:boxes[i]?.w??38,h:boxes[i]?.h??22,frame,z:10+i}));
    const sticker=(document.getElementById('pageSticker')?.value||'🐚').split(' ')[0];
    const text=(document.getElementById('pageTextBox')?.value||'').trim();
    if(sticker)elements.push({id:uid(),type:'sticker',text:sticker,x:7,y:10,w:9,h:7,z:90});
    if(text)elements.push({id:uid(),type:'text',text,x:10,y:83,w:44,h:8,z:100});
    return {id:'draft',title:document.getElementById('pageTitle')?.value.trim()||'Scrapbook Page',note:document.getElementById('pageNote')?.value.trim()||'',theme:document.getElementById('pageTheme')?.value||'Beach Day',layout,frame,bg:bgValue(),createdAt:new Date().toISOString(),elements};
  }
  function elementHtml603(p,e){
    const style=`left:${pct(e.x)};top:${pct(e.y)};width:${pct(e.w)};height:${pct(e.h)};z-index:${e.z||10}`;
    if(e.type==='photo')return `<div class="scrapItem scrapImgWrap frame-${esc(e.frame||p.frame||'soft')}" data-page="${esc(p.id)}" data-el="${esc(e.id)}" style="${style}"><img src="${e.src||''}" alt="Scrapbook photo"><span class="scrapResize" data-resize="1"></span></div>`;
    if(e.type==='sticker')return `<div class="scrapItem scrapSticker" data-page="${esc(p.id)}" data-el="${esc(e.id)}" style="${style}">${esc(e.text||'🐚')}<span class="scrapResize" data-resize="1"></span></div>`;
    return `<div class="scrapItem scrapTextBox" data-page="${esc(p.id)}" data-el="${esc(e.id)}" style="${style}">${esc(e.text||'Text')}<span class="scrapResize" data-resize="1"></span></div>`;
  }
  function toolPanel603(pageId){
    return `<div class="scrapbookToolPanel noPrint">
      <button type="button" onclick="scrapAddPhoto603('${pageId}')">Add Photo</button>
      <button type="button" onclick="scrapAddSticker603('${pageId}')">Sticker</button>
      <button type="button" onclick="scrapAddText603('${pageId}')">Text Box</button>
      <button type="button" onclick="scrapBringFront603('${pageId}')">Bring Front</button>
      <button type="button" onclick="scrapDeleteSelected603('${pageId}')">Delete Selected</button>
      <select onchange="scrapFrameSelected603('${pageId}',this.value)"><option value="soft">Soft Frame</option><option value="polaroid">Polaroid</option><option value="gold">Gold</option><option value="rose">Rose</option><option value="navy">Navy</option><option value="double">Double</option><option value="none">No Frame</option></select>
      <select onchange="scrapChangeBg603('${pageId}',this.value)"><option value="seaGlass">Sea Glass</option><option value="sand">Sand</option><option value="sunset">Sunset</option><option value="roseGold">Rose Gold</option><option value="lighthouseClean">Lighthouse</option></select>
    </div>`;
  }
  function pageHtml603(p,isDraft){
    const els=[...(p.elements||[])].sort((a,b)=>(a.z||0)-(b.z||0)).map(e=>elementHtml603(p,e)).join('');
    const buttons=isDraft
      ? `<button onclick="saveDraftScrapbookPage()" class="primary">Create Scrapbook Page</button><button onclick="cancelDraftScrapbookPage()" class="secondary">Cancel Draft</button>`
      : `<button onclick="exportScrapbookPage('${p.id}')">Export JPEG</button><button onclick="printScrapbookPage('${p.id}')">PDF / Print</button><button onclick="del('pages','${p.id}')">Delete</button>`;
    return `<div class="scrapPagePro ${isDraft?'draftPage':''}" id="page-${esc(p.id)}"><h3>${isDraft?'Rearrange Your Page':esc(p.title||'Scrapbook Page')}</h3><p class="layoutHint noPrint">Tap an item to select it. Drag to move. Use the gold corner to resize.</p>${toolPanel603(p.id)}<div class="scrapCanvas bg-${esc(p.bg||'seaGlass')}" data-page="${esc(p.id)}"><div class="scrapTitle">${esc(p.title||'Scrapbook Page')}</div>${els}${p.note?`<div class="scrapNote">${esc(p.note)}</div>`:''}</div><div class="scrapToolbar noPrint">${buttons}</div></div>`;
  }
  function getPage603(id){return String(id)==='draft'?window.draftPage:(data.pages||[]).find(p=>String(p.id)===String(id));}
  function selectedElement603(id){
    const el=document.querySelector(`.scrapItem.selected[data-page="${CSS.escape(String(id))}"]`);
    if(!el)return null;
    const p=getPage603(id); if(!p)return null;
    return (p.elements||[]).find(e=>String(e.id)===String(el.dataset.el));
  }
  function bindDrag603(){
    document.querySelectorAll('.scrapItem').forEach(el=>{
      el.onpointerdown=(ev)=>{
        if(ev.button!==0)return; ev.preventDefault();
        const canvas=el.closest('.scrapCanvas'); if(!canvas)return;
        const p=getPage603(el.dataset.page); if(!p)return;
        const item=(p.elements||[]).find(x=>String(x.id)===String(el.dataset.el)); if(!item)return;
        document.querySelectorAll('.scrapItem.selected').forEach(x=>x.classList.remove('selected'));
        el.classList.add('selected');
        const rect=canvas.getBoundingClientRect(); const resizing=ev.target.dataset.resize==='1';
        const sx=ev.clientX, sy=ev.clientY, start={x:item.x,y:item.y,w:item.w,h:item.h};
        try{el.setPointerCapture(ev.pointerId);}catch(e){}
        el.onpointermove=(mv)=>{
          const dx=(mv.clientX-sx)/rect.width*100, dy=(mv.clientY-sy)/rect.height*100;
          if(resizing){item.w=Math.max(6,Math.min(92,start.w+dx)); item.h=Math.max(4,Math.min(82,start.h+dy));}
          else {item.x=Math.max(0,Math.min(98,start.x+dx)); item.y=Math.max(0,Math.min(96,start.y+dy));}
          el.style.left=pct(item.x); el.style.top=pct(item.y); el.style.width=pct(item.w); el.style.height=pct(item.h);
        };
        el.onpointerup=()=>{el.onpointermove=null;el.onpointerup=null;if(p.id!=='draft')save();};
      };
    });
  }
  function renderDraft603(){
    let box=document.getElementById('scrapbookDraftArea');
    const anchor=document.getElementById('scrapbookMemoryChoices')||document.getElementById('scrapbookPages');
    if(!box&&anchor){box=document.createElement('div');box.id='scrapbookDraftArea';box.className='scrapbookDraftPanel noPrint';anchor.insertAdjacentElement('afterend',box);}
    if(!box)return;
    if(!window.draftPage){box.classList.add('hidden');box.innerHTML='';return;}
    box.classList.remove('hidden');
    box.innerHTML=`<h3>Step 2: Rearrange Before Saving</h3><p>Move photos, add stickers/text, resize items, then tap <b>Create Scrapbook Page</b>.</p>${pageHtml603(window.draftPage,true)}`;
    bindDrag603();
  }
  window.createDraftScrapbookPage=function(){window.draftPage=makeDraft603(); if(window.draftPage){renderDraft603();toast('Draft created. Rearrange it before saving.');}};
  window.saveDraftScrapbookPage=function(){
    if(!window.draftPage)return toast('Create a draft page first.');
    const page=JSON.parse(JSON.stringify(window.draftPage));
    page.id=uid(); page.createdAt=new Date().toISOString();
    page.photos=(page.elements||[]).filter(e=>e.type==='photo').map(e=>e.src).filter(Boolean);
    data.pages.push(page); window.draftPage=null; selectedScrapPhotos=[];
    ['pageTitle','pageNote','pageTextBox'].forEach(i=>{const el=document.getElementById(i); if(el)el.value='';});
    save(); render(); show('scrapbook'); toast('Scrapbook page saved with photos.');
  };
  window.cancelDraftScrapbookPage=function(){window.draftPage=null;renderDraft603();toast('Draft canceled');};
  window.renderScrapbookChoices=function(){
    const box=document.getElementById('scrapbookMemoryChoices'); if(!box)return;
    const imgs=allMemoryPhotos();
    const count=`<p class="selectedCount">${(selectedScrapPhotos||[]).length} photo${(selectedScrapPhotos||[]).length===1?'':'s'} selected.</p>`;
    box.innerHTML=imgs.length?`${count}<div class="memorySelect">${imgs.map(x=>`<button type="button" class="memoryChoice ${(selectedScrapPhotos||[]).includes(x.id)?'selected':''}" onclick="toggleScrapPhoto('${x.id}')"><img src="${x.src}" alt="${esc(x.name)}"><small>${esc(x.memoryTitle)}</small></button>`).join('')}</div><div class="buttonRow"><button id="createDraftPageBtn" class="primary" type="button">Create Draft Page</button><button id="clearSelectedPhotosBtn" class="secondary" type="button">Clear Selected</button></div>`:'<p>No memory photos yet. Save photos in Memories first, then come back here.</p>';
    const c=document.getElementById('createDraftPageBtn'); if(c)c.onclick=()=>window.createDraftScrapbookPage();
    const clr=document.getElementById('clearSelectedPhotosBtn'); if(clr)clr.onclick=()=>{selectedScrapPhotos=[];window.draftPage=null;window.renderScrapbookChoices();renderDraft603();};
  };
  window.renderPages=function(){
    const box=document.getElementById('scrapbookPages'); if(!box)return;
    box.innerHTML=(data.pages||[]).length?(data.pages||[]).map(p=>pageHtml603(p,false)).join(''):'<div class="card"><p>No scrapbook pages yet. Select photos, create a draft, rearrange, then save the page.</p></div>';
    bindDrag603();
  };
  window.scrapAddPhoto603=function(id){
    const p=getPage603(id); if(!p)return;
    const unused=allMemoryPhotos().filter(ph=>!(p.elements||[]).some(e=>e.mediaId===ph.id));
    const ph=unused[0]||allMemoryPhotos()[0]; if(!ph)return toast('Add photos in Memories first.');
    p.elements.push({id:uid(),type:'photo',src:ph.src,mediaId:ph.id,x:12,y:22,w:34,h:22,frame:p.frame||'soft',z:200+p.elements.length});
    if(p.id==='draft')renderDraft603(); else {save();renderPages();}
  };
  window.scrapAddSticker603=function(id){const p=getPage603(id);if(!p)return;const s=prompt('Sticker emoji or word','⭐');if(!s)return;p.elements.push({id:uid(),type:'sticker',text:s,x:12,y:12,w:10,h:8,z:220+p.elements.length});p.id==='draft'?renderDraft603():(save(),renderPages());};
  window.scrapAddText603=function(id){const p=getPage603(id);if(!p)return;const s=prompt('Text box words','Family memories');if(!s)return;p.elements.push({id:uid(),type:'text',text:s,x:12,y:78,w:42,h:8,z:230+p.elements.length});p.id==='draft'?renderDraft603():(save(),renderPages());};
  window.scrapBringFront603=function(id){const e=selectedElement603(id);const p=getPage603(id);if(!e||!p)return toast('Tap an item first.');e.z=300+(p.elements||[]).length+Math.floor(Math.random()*99);p.id==='draft'?renderDraft603():(save(),renderPages());};
  window.scrapDeleteSelected603=function(id){const p=getPage603(id);const e=selectedElement603(id);if(!e||!p)return toast('Tap an item first.');p.elements=(p.elements||[]).filter(x=>x.id!==e.id);p.id==='draft'?renderDraft603():(save(),renderPages());};
  window.scrapFrameSelected603=function(id,frame){const e=selectedElement603(id);const p=getPage603(id);if(!p)return;if(!e){p.frame=frame;(p.elements||[]).filter(x=>x.type==='photo').forEach(x=>x.frame=frame);}else if(e.type==='photo'){e.frame=frame;}p.id==='draft'?renderDraft603():(save(),renderPages());};
  window.scrapChangeBg603=function(id,bg){const p=getPage603(id);if(!p)return;p.bg=bg;p.id==='draft'?renderDraft603():(save(),renderPages());};

  const oldRender603=render;
  render=function(){oldRender603();try{window.renderScrapbookChoices();renderDraft603();window.renderPages();}catch(e){console.warn(e);}setSplashClass();};
  setTimeout(()=>{try{window.renderScrapbookChoices();renderDraft603();window.renderPages();setSplashClass();}catch(e){console.warn(e);}},300);
})();
