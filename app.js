...
const SECURITY_RULES_VERSION='10.0.5-navigation-dock-all-updates';
...
async function uploadOne(f,prefix='media',tripId=selectedTrip){if(tripId&&blockUnauthorizedTrip(tripId))throw new Error('Access denied');let obj={id:id(),tripId:tripId||'',owner:uid,name:f.name,type:f.type,createdAt:Date.now()};if(fb&&window.firebase&&firebase.storage){let path=`families/${familyId()}/${prefix}/${obj.id}-${f.name}`;let snap=await firebase.storage().ref(path).put(f);obj.url=await snap.ref.getDownloadURL()}else obj.url=await fileToDataURL(f);return obj}
...
async function uploadMedia(){let files=[...($('#mediaFile')?.files||[])],tripId=$('#mediaTripTag')?.value||'';if(!files.length){toast('Choose at least one photo or video');return}if(files.length>MAX_PHOTO_UPLOAD){toast(`Choose no more than ${MAX_PHOTO_UPLOAD} files at one time`);return}for(const f of files){arr('media').unshift(await uploadOne(f,'media',tripId))}save();activity(`Added ${files.length} media file(s)${tripId?' to a trip':' to shared family media'}`,'media');render()}
...
function exportData(){let a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(data,null,2)],{type:'application/json'}));a.download=(templateConfig().shortName||'family-app').toLowerCase().replace(/[^a-z0-9]+/g,'-')+'-backup-'+VERSION+'.json';a.click()}
