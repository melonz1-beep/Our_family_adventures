...
const SECURITY_RULES_VERSION='10.0.6-backup-restore-manager';
const DEFAULT_FAMILY='default-family';
const MAX_PHOTO_UPLOAD=5;
const LS='ofa-9-data';
const DEFAULT_ADMIN_EMAIL='mlehr1211@gmail.com';
...
const blank=()=>({version:VERSION,people:[],trips:[],memories:[],media:[],scrapbooks:[],meals:[],grocery:[],packing:[],links:[],reservations:[],budgets:[],itinerary:[],attendees:[],rsvps:[],invites:[],notifications:[],activity:[],chats:[],votes:[],profiles:[],settings:{theme:'light',adminCode:'1218',notifications:{chat:true,votes:true,trips:true,lists:true,memories:true,media:true,admin:true},template:{appName:'Our Family Adventures',shortName:'Adventures',tagline:"Life is better when we're making memories together.",heroTitle:'Making Memories Together',heroText:'',enterText:'Begin Our Journey',familyId:'default-family',adminEmail:'mlehr1211@gmail.com',primaryColor:'#d9a39a',accentColor:'#b97878',features:{chat:true,media:true,scrapbook:true,voting:true,budgets:true,maps:true,weather:true}}},locks:{},migrated:false});
let data=load();
...
function adminEmail(){return String(templateConfig().adminEmail||DEFAULT_ADMIN_EMAIL).trim().toLowerCase()}
...
function setIdentity(u){const who=$('#who');if(u){uid=u.uid;const email=(u.email||'').trim().toLowerCase();localStorage.setItem('ofa-uid',uid);localStorage.setItem('ofa-email',email);let profile=arr('people').find(p=>normIdentity(p.email)===normIdentity(email)||p.firebaseUid===u.uid||p.authUid===u.uid||p.owner===u.uid);role=email===adminEmail()?'Admin':(['Family','Guest','Child','Admin'].includes(profile?.role)?profile.role:'Family');localStorage.setItem('ofa-role',role);if(who)who.textContent=(u.email||'Signed in')+' · '+role;}else{uid='local-user';role='Family';localStorage.setItem('ofa-role','Family');if(who)who.textContent='Signed out';}}
async function linkAuthenticatedProfile(u){
  ...
  try{
    ...
    if(!person){
      ...
      person={id:id(),name:fallbackName,email:u.email,role:u.email===adminEmail()?'Admin':(invite?.role||'Family'),createdAt:now};
    ...
    if(u.email===adminEmail()&&!person.name)desired.name='Melissa Lehr';
    if(u.email===adminEmail()&&person.role!=='Admin')desired.role='Admin';
...
function showDeviceNotification(text,tag='ofa-update'){if(!('Notification'in window)||Notification.permission!=='granted')return;try{navigator.serviceWorker?.ready.then(reg=>reg.showNotification(appName(),{body:text,icon:'./icons/icon-192.png',badge:'./icons/badge-96.png',tag,renotify:true})).catch(()=>{})}catch(e){console.warn('Notification display skipped',e)}}
...
function activity(text,kind='trips',details={}){arr('activity').unshift({id:id(),text,actorName:currentPersonName(),at:Date.now(),user:uid,kind,tripId:details.tripId||selectedTrip||''});data.activity=data.activity.slice(0,120);notifyFamily(text,kind,details)}
...
function openChatWindow(){const url=new URL('./chat.html?v='+VERSION,location.href).href;const a=document.createElement('a');a.href=url;a.target='_blank';a.rel='noopener noreferrer external';document.body.appendChild(a);a.click();a.remove();toast(/iPad|iPhone|iPod/.test(navigator.userAgent)?'Chat opened in Safari':'Chat opened in a separate browser window');}
...
storageArchive(){if(!adminIsUnlocked())return views.admin();let media=arr('media').filter(m=>m.url),bytes=media.reduce((n,m)=>n+(Number(m.size)||0),0),completed=arr('trips').filter(t=>t.completed),archived=completed.filter(t=>['archived','external-only'].includes(t.archiveStatus));let health=bytes<500*1024*1024?'Excellent':bytes<2*1024**3?'Good':bytes<5*1024**3?'Consider archiving':'Archive recommended';return card('Storage & Archive Center',`<div class="storageHealth"><b>Storage health: ${health}</b><span>${app.formatBytes(bytes)} recorded across ${media.length} photos</span></div><div class="grid storageStats"><div class="card"><b>${completed.length}</b><span>Completed trips</span></div><div class="card"><b>${archived.length}</b><span>Archived trips</span></div><div class="card"><b>${media.filter(m=>m.archiveCopy).length}</b><span>Archived copies</span></div><div class="card"><b>${media.filter(m=>!m.archiveCopy).length}</b><span>Original photos</span></div></div><p class="muted">Storage totals are based on file sizes recorded by the app. Older uploads may not include a recorded size.</p>${completed.map(t=>{let rows=media.filter(m=>m.tripId===t.id),size=rows.reduce((n,m)=>n+(Number(m.size)||0),0);return `<div class="archiveManagerRow"><div><b>${escape(t.name||'Trip')}</b><small>${rows.length} photos · ${app.formatBytes(size)} · ${escape(t.archiveStatus||'ready')}</small></div><button onclick="app.openArchiveManager('${t.id}')">Manage</button></div>`}).join('')||'<p>No completed trips are ready to archive.</p>'}`)},
...
function limitScrapSelection(changed){
  const existing=scrapObjs().filter(o=>o.img).length;
  const checked=[...document.querySelectorAll('input[name="scrapPhoto"]:checked')];
  const allowed=Math.max(0,MAX_PHOTO_UPLOAD-existing);
  if(checked.length<=allowed)return true;
  if(changed)changed.checked=false;
  toast(existing?`You can select only ${allowed} more photo${allowed===1?'':'s'} for this page`:`Select no more than ${MAX_PHOTO_UPLOAD} photos`);
  return false;
}
...
function openScrapEditor(){document.querySelector('#drawer')?.classList.remove('open');let p=$('#scrapEditorPanel');if(!p)return;p.querySelectorAll('button').forEach(b=>b.type='button');p.classList.add('open');p.setAttribute('aria-hidden','false');document.body.classList.add('scrapEditing');window.__pendingSplitRefresh=false;if(!scrapObjs().length)loadScrapDraft();setTimeout(()=>{drawScrap();fitScrapCanvas();document.querySelector('.scrapSidebar')?.scrollTo({top:0})},40)}
function closeScrapEditor(){saveScrapDraft(true);let p=$('#scrapEditorPanel');if(!p)return;p.classList.remove('open');p.setAttribute('aria-hidden','true');document.body.classList.remove('scrapEditing');window.__pendingSplitRefresh=false}
...
function addSelectedPhoto(){let existing=scrapObjs().filter(o=>o.img).length;let remaining=Math.max(0,MAX_PHOTO_UPLOAD-existing);let sel=[...document.querySelectorAll('input[name="scrapPhoto"]:checked')].map(o=>o.value);if(!remaining){toast(`A scrapbook page can contain no more than ${MAX_PHOTO_UPLOAD} photos`);return}if(sel.length>remaining){toast(`You can add only ${remaining} more photo${remaining===1?'':'s'} to this page`);return}let added=0;sel.forEach((mid,i)=>{let m=arr('media').find(x=>x.id===mid);if(m&&m.url){scrapObjs().push({img:m.url,frame:$('#scrapFrame')?.value||'nofill',cutout:$('#scrapCutout')?.value||'none',frameColor:$('#scrapFrameColor')?.value||'#b97878',frameThickness:Number($('#scrapFrameThickness')?.value||6),imgX:0,imgY:0,x:(($('#scrapCanvas')?.clientWidth||680)-210)/2+(i*12),y:(($('#scrapCanvas')?.clientHeight||880)-165)/2+(i*12),w:210,h:165,z:Date.now()+i,fit:'cover',zoom:1,rot:0});added++}});if(!added){toast('Select at least one photo');return}drawScrap();document.querySelectorAll('input[name="scrapPhoto"]:checked').forEach(x=>x.checked=false);saveScrapDraft(true);setTimeout(()=>{document.body.classList.add('scrapEditing');$('#scrapEditorPanel')?.classList.add('open');fitScrapCanvas()},0)}
...
function completeTrip(tripId){const t=arr('trips').find(x=>x.id===tripId);if(!t)return;if(!canEdit(t)){toast('Only the trip creator or Admin can complete this trip');return}if(!confirm(`Mark ${t.name||'this trip'} completed and move it to Trip History?`))return;t.completed=true;t.status='completed';t.completedAt=Date.now();t.completedBy=uid;t.archiveStatus=t.archiveStatus||'ready';t.archiveMarkedAt=t.archiveMarkedAt||0;save();activity(`Completed trip: ${t.name||'Trip'}`,'trips');notifyFamily(`${currentPersonName()} completed ${t.name||'a trip'}`,'trips',t.id);toast('Trip moved to History');setRoute('history')}
function reopenTrip(tripId){const t=arr('trips').find(x=>x.id===tripId);if(!t||!canEdit(t))return;t.completed=false;t.status='active';t.reopenedAt=Date.now();delete t.completedAt;delete t.completedBy;save();activity(`Reopened trip: ${t.name||'Trip'}`,'trips');toast('Trip moved back to active Trips');render()}
...
function readWizardConfig(){const val=id=>$('#'+id)?.value?.trim()||'';return {appName:val('wizAppName')||'My Family App',shortName:val('wizShortName')||'Family App',tagline:val('wizTagline'),enterText:val('wizEnterText')||'Enter App',heroTitle:val('wizHeroTitle'),heroText:val('wizHeroText'),familyId:(val('wizFamilyId')||DEFAULT_FAMILY).toLowerCase().replace(/[^a-z0-9_-]+/g,'-'),adminEmail:val('wizAdminEmail').toLowerCase()||DEFAULT_ADMIN_EMAIL,primaryColor:val('wizPrimary')||'#d9a39a',accentColor:val('wizAccent')||'#b97878',features:Object.fromEntries(['chat','media','scrapbook','voting','budgets','maps','weather'].map(k=>[k,!!$('#wizFeature_'+k)?.checked]))}}
...
function downloadTripOriginals(tripId){const rows=arr('media').filter(m=>m.tripId===tripId&&!m.archiveCopy&&m.url);if(!rows.length){toast('No original photos found');return}rows.forEach((m,i)=>setTimeout(()=>{const a=document.createElement('a');a.href=m.url;a.download=m.name||`photo-${i+1}.jpg`;a.target='_blank';a.rel='noopener';document.body.appendChild(a);a.click();a.remove()},i*350));toast(`Started ${rows.length} photo download${rows.length===1?'':'s'}`)}
