...
function purgeAssets(pageId){return new Promise(resolve=>{try{const request=indexedDB.open(ASSET_DB,1);request.onerror=()=>resolve(false);request.onupgradeneeded=()=>{if(!request.result.objectStoreNames.contains(ASSET_STORE))request.result.createObjectStore(ASSET_STORE)};request.onsuccess=()=>{const db=request.result,tx=db.transaction(ASSET_STORE,'readwrite'),store=tx.objectStore(ASSET_STORE),cursor=store.openCursor();cursor.onsuccess=()=>{const current=cursor.result;if(!current)return;if(String(current.key).startsWith(`${pageId}:`))current.delete();current.continue()};tx.oncomplete=()=>{db.close();resolve(true)};tx.onerror=()=>{db.close();resolve(false)}}}catch{resolve(false)}})}
...
function remotePaths(job){
 ...
 const recordIds=new Set([job.recordId,job.pageId].filter(Boolean));recordIds.forEach(recordId=>{paths.add(`families/${family}/publicData/shared/scrapbooks/${recordId}`);if(job.tripId){paths.add(`families/${family}/familyTrips/${job.tripId}/collections/scrapbooks/${recordId}`);paths.add(`families/${family}/privateTrips/${job.tripId}/collections/scrapbooks/${recordId}`)}});
 return [...paths];
}
...
async function deletePage(info,row){
 ...
 history.replaceState(null,'',`${location.pathname}${location.search}#scrapbook`);location.reload();
}
