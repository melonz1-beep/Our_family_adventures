const CACHE='ofa-9-1-1';
const CORE=['./','./index.html','./404.html','./style.css?v=9.1.1','./app.js?v=9.1.1','./manifest.json','./firebase-config.js','./assets/lighthouse-home.png','./icons/icon-192.png','./icons/icon-512.png','./icons/badge-96.png'];
...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'){
    event.respondWith(fetch(event.request).then(r=>{if(r.ok)caches.open(CACHE).then(c=>c.put('./index.html',r.clone()));return r;}).catch(()=>caches.match('./index.html')));
    return;
  }
