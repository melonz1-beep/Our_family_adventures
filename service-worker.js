...
try {
  ...
  fcmMessaging.onBackgroundMessage(payload => {
    ...
    return self.registration.showNotification(n.title || d.title || 'Our Family Adventures', {
      ...
      renotify: true,
      data: { url: d.url || './index.html?v=9.3.5#notifications' }
    });
  });
} catch (error) {
...
const CACHE='ofa-9-3-5';
const OFFLINE='./index.html?v=9.3.5';
const CORE=['./','./index.html?v=9.3.5','./404.html','./style.css?v=9.3.5','./manifest.json?v=9.3.5','./firebase-config.js?v=9.3.5','./assets/lighthouse-home.png','./icons/icon-192.png','./icons/icon-512.png','./icons/badge-96.png'];
...
self.addEventListener('push',event=>{let d={};try{d=event.data?event.data.json():{}}catch(_){d={body:event.data?.text()||'Family adventure update'}}const n=d.notification||d;event.waitUntil(self.registration.showNotification(n.title||'Our Family Adventures',{body:n.body||d.data?.body||'You have a new family update.',icon:n.icon||'./icons/icon-192.png',badge:'./icons/badge-96.png',tag:d.data?.tag||n.tag||'ofa-push',renotify:true,data:{url:d.data?.url||'./index.html?v=9.3.5#notifications'}}))});
self.addEventListener('notificationclick',event=>{event.notification.close();event.waitUntil(clients.matchAll({type:'window',includeUncontrolled:true}).then(list=>{const url=event.notification.data?.url||'./index.html?v=9.3.5#notifications';for(const c of list){if('focus'in c){c.navigate(url);return c.focus()}}return clients.openWindow(url)}))});
