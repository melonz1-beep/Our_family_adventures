...
const completeCss = readFileSync(new URL('../scrapbook-complete-10-3-17.css', import.meta.url), 'utf8');
...
const themes = ['pet-memories-realistic.svg','wedding-romance-realistic.svg','happy-hour-realistic.svg','baby-keepsake-realistic.svg','winter-wonderland-realistic.svg','chalkboard-memories-realistic.svg','vintage-scroll-realistic.svg','warm-sand-realistic.svg','soft-paisley-realistic.svg','christmas-realistic.svg','halloween-realistic.svg','easter-realistic.svg','patriotic-realistic.svg','sea-glass-realistic.svg'];
...
test('premium scrapbook browser layers parse', () => {
 ...
 assert.doesNotThrow(() => new Script(complete));
});
...
test('editor menu follows title, backgrounds, frames, photos, text, and layouts', () => {
 ...
 let previous = -1;
 for (const label of labels) { const position = premium.indexOf(label); assert.ok(position > previous, `${label} is missing or out of order`); previous = position; }
...
test('dimensional scrapbook frames and final smooth masks are available', () => {
 ...
 assert.match(complete, /heart:maskSvg/);
 assert.match(complete, /flower:maskSvg/);
 assert.match(complete, /puzzle:maskSvg/);
 assert.match(completeCss, /ss2-shell-real/);
});
...
test('shadow and glow values persist and render outside the clipped photo frame', () => {
 ...
 assert.match(complete, /drop-shadow/);
 assert.match(complete, /applyEffects/);
 assert.match(completeCss, /\.ss2-object\{overflow:visible!important\}/);
});
...
test('10.3.17 loads premium, scene-safe, and complete layers in order', () => {
 assert.match(index, /scrapbook-premium-10-3-15\.css\?v=10\.3\.17/);
 assert.match(index, /scrapbook-complete-10-3-17\.css\?v=10\.3\.17/);
 assert.match(index, /scrapbook-premium-10-3-15\.js\?v=10\.3\.17/);
 assert.match(index, /scrapbook-scenes-10-3-16\.js\?v=10\.3\.17/);
 assert.match(index, /scrapbook-complete-10-3-17\.js\?v=10\.3\.17/);
 assert.ok(index.indexOf('scrapbook-complete-10-3-17.js') > index.indexOf('scrapbook-scenes-10-3-16.js'));
...
test('release version and PWA cache are consistently 10.3.17', () => {
 ...
 assert.match(worker, /scrapbook-complete-10-3-17\.js/);
